...
- GitHub Actions 默认输出正式签名 APK，只通过 GitHub Secrets 读取签名材料，仓库内禁止提交 keystore、密码、签名后的派生产物。
...
Android release APK 只构建 `arm64-v8a`：

- `Android/app/build.gradle.kts` 必须在 `defaultConfig.ndk.abiFilters` 中固定 `arm64-v8a`。
- GitHub Actions 和本地 release 构建都必须沿用同一份 Gradle 配置，不允许在 CI 中临时覆盖成全架构包。
- 验收时需要检查 APK 内 native library 路径，只允许出现 `lib/arm64-v8a/`。
- 如后续需要兼容 x86_64 模拟器或 armeabi-v7a 老设备，必须单独开新 artifact 名称，不能复用 `NeoRemote-android-release-signed.apk`。

当前 NeoRemote release keystore 已创建在本机私有目录：

- Keystore：`/Users/ebato/.neoremote/android-release-signing/NeoRemote-release.jks`
- 本地环境变量文件：`/Users/ebato/.neoremote/android-release-signing/credentials.env`
- Key alias：`neoremote-release`

`credentials.env` 只用于本机恢复签名环境，权限应保持 `600`，不要复制到仓库目录。
...
```bash
cd /Users/ebato/Documents/Projects/NeoRemote
source "$HOME/.neoremote/android-release-signing/credentials.env"
...
当前仓库 `Souitou-iop/NeoRemote` 已配置以上 4 个 Secrets。后续轮换 keystore 或密码时，必须同步更新这 4 个 Secrets。
...
`.github/workflows/build-mobile.yml` 的 Android job 必须遵循：

- 先执行 `./gradlew assembleRelease` 生成未签名 APK。
- 从 `ANDROID_RELEASE_KEYSTORE_BASE64` 还原 keystore 到 `$RUNNER_TEMP`。
- 使用 `apksigner sign` 输出 `artifacts/NeoRemote-android-release-signed.apk`。
- 使用 `apksigner verify --verbose --print-certs` 校验正式签名。
- 上传 artifact 名称必须是 `NeoRemote-android-release-signed-apk`。
