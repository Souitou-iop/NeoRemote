...
NeoRemote 是一个四端原生的跨设备输入项目。当前阶段聚焦“移动端控制桌面端”：手机负责发现 Desktop、建立连接并采集触控输入；桌面端负责监听连接、解析统一协议，并把输入注入为系统鼠标事件。后续会在同一连接模型上扩展“移动端互控”，让手机、平板与桌面端之间共享更一致的发现、连接和输入通道。
...
| Android | Kotlin / Jetpack Compose / 原生 Socket / Android NSD | 移动控制端，已接入同源应用图标 |
...
| 移动端互控 | 复用发现、会话和输入协议，扩展手机/平板之间的控制链路 | 已预留 |
